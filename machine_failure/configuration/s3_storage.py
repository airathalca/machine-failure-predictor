...
class S3Storage:
  ...
  def s3_key_path_available(self, bucket_name, s3_key) -> bool:
    ...
    try:
      ...
      logging.info(f"Exiting the s3_key_path_available method of S3Storage class")
  ...
  def read_object(object_name: str, decode: bool = True, make_readable: bool = False) -> Union[StringIO, str]:
    logging.info("Entered the read_object method of S3Storage class")
    try:
      func = (
      ...
      logging.info("Exiting the read_object method of S3Storage class")
      return conv_func()
    except Exception as e:
  ...
  def get_bucket(self, bucket_name: str) -> Bucket:
    logging.info("Entered the get_bucket method of S3Storage class")
    try:
      bucket = self.s3_resource.Bucket(bucket_name)
      logging.info("Exiting the get_bucket method of S3Storage class")
      return bucket
    except Exception as e:
  ...
  def get_file_object(self, filename: str, bucket_name: str) -> Union[List[object], object]:
    logging.info("Entered the get_file_object method of S3Storage class")
    try:
      bucket = self.get_bucket(bucket_name)
      file_objects = [file_object for file_object in bucket.objects.filter(Prefix=filename)]
      func = lambda x: x[0] if len(x) == 1 else x
      file_objs = func(file_objects)
      logging.info("Exiting the get_file_object method of S3Storage class")
      return file_objs
    except Exception as e:
  ...
  def load_model(self, model_name: str, bucket_name: str, model_dir: str = None) -> object:
    logging.info("Entered the load_model method of S3Storage class")
    try:
      ...
      logging.info("Exiting the load_model method of S3Storage class")
      return model
    except Exception as e:
  ...
  def create_folder(self, folder_name: str, bucket_name: str) -> None:
    logging.info("Entered the create_folder method of S3Storage class")
    try:
      ...
      if e.response["Error"]["Code"] == "404":
        ...
        pass
      logging.info("Exiting the create_folder method of S3Storage class")
    except Exception as e:
      logging.error(f"Error in cls S3Storage method create_folder: {e}")
      raise CustomException(e, sys)

  def upload_file(self, local_filename: str, bucket_filename: str,  bucket_name: str,  remove: bool = True):
    logging.info("Entered the upload_file method of S3Storage class")
    try:
      logging.info(
      ...
      logging.info("Exiting the upload_file method of S3Storage class")
    except Exception as e:
  ...
  def upload_df_as_csv(self,df: pd.DataFrame, local_filename: str, bucket_filename: str, bucket_name: str) -> None:
    logging.info("Entered the upload_df_as_csv method of S3Storage class")
    try:
      df.to_csv(local_filename, index=None, header=True)
      self.upload_file(local_filename, bucket_filename, bucket_name)
      logging.info("Exiting the upload_df_as_csv method of S3Storage class")
    except Exception as e:
      logging.error(f"Error in cls S3Storage method upload_df_as_csv: {e}")
      raise CustomException(e, sys)

  def get_df_from_object(self, object_: object) -> pd.DataFrame:
    logging.info("Entered the get_df_from_object method of S3Storage class")
    try:
      content = self.read_object(object_, make_readable=True)
      df = pd.read_csv(content, na_values="na")
      logging.info("Exiting the get_df_from_object method of S3Storage class")
      return df
    except Exception as e:
  ...
  def read_csv(self, filename: str, bucket_name: str) -> pd.DataFrame:
    logging.info("Entered the read_csv method of S3Storage class")
    try:
      csv_obj = self.get_file_object(filename, bucket_name)
      df = self.get_df_from_object(csv_obj)
      logging.info("Exiting the read_csv method of S3Storage class")
      return df
    except Exception as e:
